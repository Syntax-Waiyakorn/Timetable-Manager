...
Public Class MixmizeTable
    ...
    Sub InitializeTable()
        ...
        conn.Close()
    End Sub
    Sub DisplayClassroomTable(Classroom)
        ...
        Next
    End Sub
    Sub DisplayPeriod(Classroom, PLabel)
        Try
            conn.Open()
            Dim PLabelName As String = PLabel.Name
            Dim PDay As String = PLabelName.Chars(4)
            Dim PPeriod As String = PLabelName.Chars(6)
            Dim cmd As New OleDb.OleDbCommand("Select * from Timetable WHERE  'ClassroomName' like '%" & CStr(Classroom) & "%' and `DayNumber` like '%" & CStr(PDay) & "%' and `PeriodNumber` like '%" & CStr(PPeriod) & "%'", conn)
            dr = cmd.ExecuteReader
            While dr.Read
    ...
    Private Sub cboClassrooms_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboClassrooms.SelectedIndexChanged
        Dim Classroom As String = CStr(cboClassrooms.SelectedText)
